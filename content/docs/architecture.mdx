...
│                                                          │
│  13 database tables    Vector indexes (memories)        │
...
All 45 tools are stateless from the MCP server's perspective — state lives in Convex. This means you can restart the MCP server process at any time without losing data.
