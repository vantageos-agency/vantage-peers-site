...
│                                                          │
│  10 database tables    Vector indexes (memories)        │
...
VantagePeers uses 10 Convex tables. Each table has a defined schema with typed validators and indexes for efficient querying.
...
All 37 tools are stateless from the MCP server's perspective — state lives in Convex. This means you can restart the MCP server process at any time without losing data.
